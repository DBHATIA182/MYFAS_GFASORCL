...
echo [1/4] Stopping ALL cloudflared on this PC...
taskkill /F /IM cloudflared.exe >nul 2>&1

echo [2/4] Stopping anything listening on 5173 (Vite) and 5001 (API)...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ports = @(5173, 5001); foreach ($port in $ports) { Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { Write-Host ('  PID ' + $_ + ' on port ' + $port); Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue } }"

echo [3/4] Waiting 2 seconds...
timeout /t 2 /nobreak >nul

echo [4/4] Starting API, Vite (host 0.0.0.0:5173), tunnel (.\config.yml)...
start "GFASORCL-API" /min /D "%APP%" cmd /k "node server.cjs"
timeout /t 2 /nobreak >nul

<#
.SYNOPSIS
  Schedule automatic git pull + build on the GFASORCL client PC (daily and/or at logon).
...
Right-click setup-gfasorcl-git-update.cmd -> Run as administrator
'@
}
...
$legacyTasks = @(Get-ScheduledTask -TaskPath '\' -ErrorAction SilentlyContinue |
    Where-Object { $_.TaskName -like 'GFAS-*-GitUpdate' -or $_.TaskName -like 'FAS-*-GitUpdate' })
...
$desc = ('GFASORCL: stop services, git pull + npm build, restart. Daily at {0}' -f $DailyTime)
if ($AtLogon) {
...
Write-Host '  1. Stop node, cloudflared, ports 5001/5002/5173, autostart scheduled tasks'
Write-Host '  2. git pull + npm ci + build'
Write-Host '  3. Restart via start-apptest-services.ps1'
Write-Host ''
Write-Host 'Dev PC workflow:' -ForegroundColor Cyan
Write-Host '  1. push-all-to-git.ps1 on E:\GFASORCL\APPTEST'
Write-Host '  2. Client pulls automatically (this task) or run update-from-git.ps1'
Write-Host ''

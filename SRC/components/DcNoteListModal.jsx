...
const LIST_COLUMNS = [
  ...
  { key: 'bill_amt', label: 'Amount', align: 'right' },
];
...
function noteLabel(noteType) {
  return String(noteType || '').toUpperCase() === 'CX' ? 'Credit Note' : 'Debit Note';
}

/** List modal for DCNOTE DX / CX. */
export default function DcNoteListModal({
  open,
  apiBase,
  apiParams,
  noteType = 'DX',
  ...
  const type = String(noteType || 'DX').trim().toUpperCase() === 'CX' ? 'CX' : 'DX';
  ...
  const runSearch = useCallback(
    async (includeDates = true, overrides = null) => {
      ...
      try {
        const { data } = await axios.get(`${apiRoot}/api/dc-note/list`, {
          params: {
            ...apiParams,
            type,
    ...
    [apiParams, apiRoot, sdt, edt, party, type]
  );
  ...
  }, [open, fyMinYmd, fyMaxYmd, type]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div
      className="voucher-help-modal voucher-help-modal--open"
      role="dialog"
      aria-modal="true"
      aria-label={`${noteLabel(type)} list`}
    >
      <button type="button" className="voucher-help-modal__backdrop" aria-label="Close" onClick={onClose} />
      <div className="voucher-help-modal__panel dc-list-modal">
        <div className="voucher-help-modal__head">
          <h3 className="voucher-help-modal__title">{noteLabel(type)} List</h3>
          <button type="button" className="btn btn-sm" onClick={onClose}>
            Close
          </button>
        </div>
        <div className="dc-list-modal__filters">
          <label>
            From
            <input
              ref={fromDateRef}
        ...
        <div className="voucher-help-modal__body dc-list-modal__body">
          <table className="voucher-help-modal__table">
            ...
            <tbody>
              ...
              {!loading && !rows.length ? (
                <tr>
                  <td colSpan={LIST_COLUMNS.length}>No {noteLabel(type).toLowerCase()}s found.</td>
                </tr>
              ) : null}
            </tbody>
          </table>

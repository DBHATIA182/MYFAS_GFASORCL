...
function LedgerTable({ rows }) {
  if (!rows.length) return <SectionEmpty text="No LEDGER rows for this note." />;
  let totDr = 0;
  let totCr = 0;
  for (const r of rows) {
...
function BillsTable({ rows }) {
  if (!rows.length) return <SectionEmpty text="No BILLS rows for this note." />;
  return (
    <table className="pb-posting-modal__table">
      <thead>
        <tr>
...
/** LEDGER / BILLS postings for DCNOTE DX / CX. */
export default function DcNotePostingModal({
  open,
  apiBase,
  apiParams,
  noteType = 'DX',
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const { data: out } = await axios.get(`${apiRoot}/api/dc-note/posting`, {
        params: {
          ...apiParams,
          type,
  ...
  }, [apiParams, apiRoot, rDate, rNo, type]);
  ...
  const okPost =
    ledger.length > 0 && bills.length > 0
      ...
      : ledger.length > 0
        ...
        : 'No posting rows yet — save the note first.';

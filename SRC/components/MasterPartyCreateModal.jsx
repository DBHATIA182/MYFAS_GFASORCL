...
export default function MasterPartyCreateModal({
  ...
  const loadNextCode = useCallback(
    async (schedVal) => {
      ...
      try {
        ...
        setCode(normalizeMasterPartyCode(data?.next_code ?? data?.NEXT_CODE ?? ''));
      } catch (ex) {
        const msg = ex?.response?.data?.error || ex.message || 'Could not suggest next code';
        setErr(msg);
        if (ex?.response?.status === 400) alert(msg);
        setCode('');
      }
    },
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      setCode(normalizeMasterPartyCode(editRow.CODE ?? editRow.code ?? ''));
  ...
  const handleSave = async (e) => {
    ...
    const payload = {
      ...
      user_name: userName,
      schedule: sch,
      code: normalizeMasterPartyCode(code) || undefined,

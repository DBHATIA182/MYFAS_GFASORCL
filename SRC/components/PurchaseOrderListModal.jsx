...
import { formatDmyTyping, formatLedgerDateDisplay, parseDmyDisplay, toDisplayDate, toOracleDate } from '../utils/dateFormat';
...
export default function PurchaseOrderListModal({
  open,
  apiBase,
  apiParams,
  fyMinYmd = '',
  ...
  const runSearch = useCallback(async (includeDates = true, overrides = null) => {
    ...
    try {
      const { data } = await axios.get(`${apiRoot}/api/purchase-order/list`, {
        params: {
          ...apiParams,
  ...
  const title = useMemo(() => {
    ...
    return `Purchase orders — ${from} to ${to}`;
  }, [sdt, edt]);

  if (!open) return null;

  return (
    <div className="voucher-help-modal" role="dialog" aria-modal="true" aria-label="Purchase order list">
      ...
      <div className="voucher-help-modal__panel voucher-help-modal__panel--account purchase-order-list-modal">
        ...
        <div className="voucher-help-modal__body voucher-help-modal__body--account">
          ...
          {!loading && !error && !rows.length ? (
            <p className="voucher-help-modal__msg">No purchase orders found.</p>
          ) : null}

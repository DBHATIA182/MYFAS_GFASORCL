...
export default function Slide56CompleteLedgerReport({ apiBase, onPrev, onReset, formData, slideClass = '' }) {
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [scheduleNo, setScheduleNo] = useState('');
  const [startCode, setStartCode] = useState('1');
  const [endCode, setEndCode] = useState('Z99999');
  ...
  const handleSubmit = async (e) => {
    ...
    if (sCode.toUpperCase().localeCompare(eCode.toUpperCase()) > 0) {
  ...
  const downloadPDF = async () => {
    if (!reportPayload?.sections?.length) {
    ...
    <div className={`slide complete-ledger-entry ${slideClass}`.trim()}>
      ...
      <SessionInfoLine formData={formData} helpReportId="complete-ledger">
        <span className="compdet-date-hint">
          Account codes are VARCHAR2 (e.g. 1, B26001, Z99999). Default range 1–Z99999. Leave schedule blank for all schedules.
        </span>
      </SessionInfoLine>
          ...
          <div className="form-row-broker complete-ledger-form-codes">
            <div className="form-group">
              ...
              <input
                ...
                type="text"
                maxLength={6}
                className="form-input complete-ledger-code-input"
                value={startCode}

...
/** VFP DO FORM brokchk WITH 1 — sale bills with brokerage % but zero brokerage amount. */
export default function Slide67BrokFind({ apiBase, formData, userName, onPrev }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setErr('');
      try {
        const { data } = await axios.get(apiUrl(apiBase, '/api/brok-find-user-permissions'), {
  ...
  const handleProceed = async () => {
    ...
    setErr('');
    try {
      const { data } = await axios.post(apiUrl(apiBase, '/api/brok-find-proceed'), buildPayload(), reqOpts);
      const rows = (Array.isArray(data?.rows) ? data.rows : []).map(mapGridRow);
      setGridRows(rows);
      if (!rows.length) alert('No sale bills found with brokerage % but zero brokerage amount.');
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="slide slide-67-brok-find brok-find-screen inttrf-screen">
        <p className="loading-msg">Loading Brok.Find…</p>
      </div>
    );
  }
  ...
  return (
    <div className="slide slide-67-brok-find brok-find-screen inttrf-screen detail-mast-screen account-master-screen">
      <SaleBillPrintModal
        open={billPrintOpen}
        onClose={() => {
      ...
      <div className="account-master-screen__head inttrf-screen__head">
        <h2 className="sale-bill-page__title inttrf-screen__title">Brok.Find</h2>
        <p className="brok-find-screen__subtitle">Broker Trial</p>
        <SessionInfoLine formData={formData} userName={userName} helpReportId="brok-find" />
      </div>
      ...
      <div className="inttrf-screen__header-panel brok-find-screen__head">
        ...
        <p className="brok-find-screen__hint">
          VFP brokchk WITH 1: sale bills (TYPE SL) with BROK_PER ≠ 0 and BROKERAGE = 0. Click a row to open the sale
          bill print view.
        </p>
      </div>
      ...
      <div className="inttrf-screen__body">
        <div className="inttrf-screen__grid-wrap">
          <table className="inttrf-screen__grid brok-find-screen__grid">
            <thead>
              <tr>
                <th>Bill No</th>
                <th>Bill Date</th>
            ...
            <tbody>
              {gridRows.length === 0 ? (
                <tr>
                  <td colSpan={13} className="inttrf-screen__grid-empty">
                ...
                gridRows.map((r) => (
                  <tr
                    key={r._id}
                    className="brok-find-row-clickable"
                    onClick={() => openBill(r)}
                    onKeyDown={(e) => {
        ...
        <div className="inttrf-screen__footer-panel">
          <div className="inttrf-screen__footer-toolbar">
            ...
            <button
              type="button"
              className="btn btn-primary inttrf-btn brok-find-screen__proceed"

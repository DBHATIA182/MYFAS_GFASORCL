...
/** VFP DO FORM update — stock/account balance updation to next year (desktop only). */
export default function Slide79Updation({ apiBase, formData, userName, onPrev }) {
  ...
  const [supTrf, setSupTrf] = useState('Y');
  const [custTrf, setCustTrf] = useState('Y');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setErr('');
      try {
        const { data } = await axios.get(apiUrl(apiBase, '/api/updation/context'), {
          params: {
            comp_code: compCode,
            comp_uid: compUid,
            user_name: userName || '',
        ...
        setMinAmt(String(c.defaultMinAmount ?? 10));
        setSupTrf(c.defaultSupTrf ?? 'Y');
        setCustTrf(c.defaultCustTrf ?? 'Y');
        if (!c.hasNextYear) {
  ...
  const handleProceed = async () => {
    if (!canProceed) {
      alert('You do not have permission to run updation.');
      return;
    }
    if (!nextDir.trim()) {
    ...
    if (
      !window.confirm(
        'Proceed with balance updation to next year schema?\n\nThis updates opening balances and may transfer bills.'
      )
    ) {
      return;
    }
    ...
    try {
      const { data } = await axios.post(
        apiUrl(apiBase, '/api/updation/execute'),
        {
          comp_code: compCode,
          comp_uid: compUid,
      ...
      alert(
        `DONE\n\nAccounts updated: ${data?.accountsUpdated ?? 0}\nBills transferred: ${data?.billsTransferred ?? 0}`
      );
    } catch (e) {
      const msg = e?.response?.data?.error || e.message || 'Updation failed';
      setErr(msg);
      alert(msg);
    } finally {
      setRunning(false);
  ...
  if (loading) {
    return (
      <div className="slide slide-79-updt updt-screen">
        <div className="updt-screen__card updt-screen__card--loading">
          <p className="loading-msg">Loading Updation…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="slide slide-79-updt updt-screen detail-mast-screen account-master-screen">
      <div className="updt-screen__card">
        <div className="account-master-screen__head inttrf-screen__head">
          <h2 className="sale-bill-page__title inttrf-screen__title">Updation</h2>
          <SessionInfoLine formData={formData} userName={userName} helpReportId="updation" />
        </div>

        {err ? <p className="form-error inttrf-screen__error">{err}</p> : null}

        <p className="updt-screen__hint">
          Stock/Account balance updation from old year to new year (VFP <code>DO FORM update</code>). Desktop
          only.
        </p>
        ...
        <Field label="Minimum Amount Ignore">
          <input
            className="inttrf-input"
            type="number"
            min="0"
        ...
        <Field label="Bills Trf Supplier (Y/N)">
          <select
            className="inttrf-input"
            value={supTrf}
            onChange={(e) => setSupTrf(e.target.value)}
            disabled={running}
          >
        ...
        <Field label="Bills Trf Customer (Y/N)">
          <select
            className="inttrf-input"
            value={custTrf}
            onChange={(e) => setCustTrf(e.target.value)}
            disabled={running}
          >
            <option value="Y">Y</option>
            <option value="N">N</option>
      ...
      {running ? (
        <p className="updt-screen__status">
          Running updation… <TransferElapsedClock elapsedMs={elapsedMs} label={elapsedLabel} />
        </p>
      ) : null}

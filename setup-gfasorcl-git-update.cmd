...
if errorlevel 1 (
  ...
  echo Right-click setup-gfasorcl-git-update.cmd -^> Run as administrator
  echo.
  pause
  exit /b 1
)
...
echo.
echo Registers automatic Git update on this PC (GFASORCL APPTEST).
echo.
echo Scheduled task name (every client): git_dal_update
echo   Test run: schtasks /Run /TN "git_dal_update"
echo.
echo Each run will:
echo   1. STOP GFAS services (API, Vite, tunnel, autostart tasks, ports 5001/5002/5173)
echo   2. git pull + npm ci + build
echo   3. RESTART services (start-apptest-services.ps1)
echo.
...
echo   setup-gfasorcl-git-update.cmd -RunNow
echo.
echo Other examples:
echo   setup-gfasorcl-git-update.cmd -NoAtLogon -DailyTime "02:00"
echo.
